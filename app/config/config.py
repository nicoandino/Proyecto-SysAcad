...
# Confirmación por consola (opcional, podés comentar si no querés logs)
...
class Config(object):
    ...
    HASHIDS_MIN_LENGTH: str | None = os.getenv('HASHIDS_MIN_LENGTH')
    HASHIDS_ALPHABET: str | None   = os.getenv('HASHIDS_ALPHABET')
    HASHIDS_SALT: str | None       = os.getenv('HASHIDS_SALT')
...
def factory(app_context: str) -> Config:
    ...
    if config_class is None:
        raise ValueError(
            f"Contexto inválido: '{app_context}'. "
            "Debe ser 'testing', 'development' o 'production'."
        )
