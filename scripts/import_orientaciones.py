...
from app.models.orientacion import OrientacionModel  # usamos el modelo existente
    ...
    with app.app_context():
        ...
        for item in root.findall('_expxml'):
            try:
                ...
                if especialidad_id is None or plan is None:
                    print("Saltado: faltan 'especialidad' o 'plan'.")
                    errores += 1
                    continue

                # Validar FK: la especialidad debe existir
                if not EspecialidadModel.query.get(especialidad_id):
                ...
                if existing:
                    print(f"Duplicado: Esp {especialidad_id}, Plan {plan}, Nombre: {nombre}")
                    duplicados += 1
                    continue

                nueva = OrientacionModel(
                    especialidad_id=especialidad_id,
                    plan=plan,
                    nombre=nombre
                )
                ...
                db.session.commit()
                insertados += 1
                print(f"Guardado Orientación ID {nueva.id}: Esp {especialidad_id}, Plan {plan}, Nombre: {nombre}")

            except IntegrityError:
                db.session.rollback()
                print(f"Error de integridad al insertar Esp {especialidad_id}, Plan {plan}")
                errores += 1
